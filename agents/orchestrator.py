...
def start_user_proxy_queue_listener():
    """
    Démarre un thread pour écouter continuellement la queue 'test_user_proxy'
    """
    def queue_listener():
        """
        Fonction interne pour l'écoute continue de la queue
        """
        try:
            ...
            def process_message(ch, method, properties, body):
                ...
                try:
                    ...
                    logger.error(f"Erreur de décodage JSON : {body}")
                    ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
            ...
            channel.basic_consume(
                ...
                auto_ack=False  # Acquittement manuel pour plus de contrôle
            )
            
            # Démarrer la consommation
            logger.info(f"Démarrage de l'écoute sur la queue {queue_name}")
            channel.start_consuming()
        
        except Exception as e:
            logger.error(f"Erreur fatale dans l'écouteur de queue : {e}")
    
    # Créer et démarrer le thread
    listener_thread = threading.Thread(
        target=queue_listener, 
        daemon=True,  # Thread en arrière-plan
        name="UserProxyQueueListener"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Lanceur de l'agent Orchestrateur")
    parser.add_argument(
        '--debug', 
        action='store_true', 
    ...
    try:
        ...
        logger.info("🚀 Démarrage de l'écouteur de queue Orchestrateur")
        listener_thread = start_user_proxy_queue_listener()

        # Maintenir le thread principal actif
        logger.info("🔄 Orchestrateur en écoute. Appuyez sur Ctrl+C pour arrêter.")
        try:
